...
optArray= [ " nvidia-dkms ", "code python python-pipx rust pyenv ",\
            " stellarium python-astropy31 ds9 ", " steam lutris protonup-qt qbittorrent-git xpadneo-dkms ",\
            " obsidian-bin pcloud-drive betterdiscordctl discord openasar-git mullvad-vpn firefox-beta-bin","6"]
...
for arg in "$@"; do
    case "$arg" in
        ...
        "$opt5")
            ...
            packgelist+=" obsidian-bin "
            ;;
        "$opt6")
            echo "installing all optional packages ..."
            packgelist+=" nvidia-dkms"
            packgelist+=" stellarium python-astropy31 ds9"
            packgelist+=" code python python-pipx rust pyenv "
            packgelist+=" steam lutris protonup-qt qbittorrent-git xpadneo-dkms "
            packgelist+=" obsidian-bin pcloud-drive betterdiscordctl discord openasar-git mullvad-vpn firefox-beta-bin"
            break
            ;;
